...
function getChangelogReleasesHeaders(splittedChangelog) {
  return splittedChangelog.filter(isReleaseHead);
}
...
function getRelease(splittedChangelog, startIndex, endIndex) {
  return splittedChangelog.slice(startIndex, endIndex).join('\n');
}
...
function getChangelogHead(splittedChangelog, lastReleaseHeaderIndex) {
  return splittedChangelog.slice(0, lastReleaseHeaderIndex - 1).join('\n');
}
...
function isChangeMessageRow(row, match) {
  return match ? match.test(row) : null;
}
...
function getMessageFromChangeMessageRow(row, messageRowPrefix) {
  return row && messageRowPrefix ? row.replace(messageRowPrefix, '') : null;
}
...
function getChangeMessageRowsFromRelease(changelogRelease, match) {
  return changelogRelease ? changelogRelease.filter((row) => isChangeMessageRow(row, match)) : null;
}
...
function getLastChangelogRelease(changelogReleases) {
  return changelogReleases ? changelogReleases[0] : null;
}
