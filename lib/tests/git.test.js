...
describe('lib/git: excludeCommitsByMatch', () => {
  test('Should return commits list without excluded commits', () => {
    const commitsList = excludeCommitsByMatch(commits, /^(\[NO-TASK\])/);

    expect(commitsList.length).toBe(3);
  });

  test('SHould return original commits list', () => {
    const commitsList = excludeCommitsByMatch(commits, /^(\[TASK-0000\])/);

    expect(commitsList.length).toBe(4);
  });

  test('Should return null if commits list is undefined', () => {
    const commitsList = excludeCommitsByMatch(undefined, /^(\[NO-TASK\])/);

    expect(commitsList).toBeNull();
  });

  test('Should return commits if regex is undefined', () => {
    const commitsList = excludeCommitsByMatch(commits, undefined);

    expect(commitsList).toEqual(commits);
  });
});
...
describe('lib/git: getCommitsByMessages', () => {
  test('Should return selected commits', () => {
    const selectedCommits = getCommitsByMessages(commits, [commit.message, commit2.message]);
    expect(selectedCommits[0].message).toEqual(commit.message);
    expect(selectedCommits[1].message).toEqual(commit2.message);
  });

  test('Should return null if commits or commitMessages is undefined', () => {
    const selectedCommits1 = getCommitsByMessages(undefined, [commit.message]);
    const selectedCommits2 = getCommitsByMessages(commits, undefined);

    expect(selectedCommits1).toBeNull();
    expect(selectedCommits2).toBeNull();
  });
});
